...
{
    if (Keya_Bus.events() == 256)
    {  
        if (dontrepeat == 0)
        {
            Serial.println("Keya Comms Lost");
            keyaEncoderInitialized = false;
            keyaDetected=false;
            dontrepeat = 1;
            Serial.println(keyaSteeringPosition);
            //keyaSteeringPosition = 0;
            //Serial.println(keyaSteeringPosition);
        }
    ...
    // added to test autoreconnect.
    //Serial.println(Keya_Bus.events());
