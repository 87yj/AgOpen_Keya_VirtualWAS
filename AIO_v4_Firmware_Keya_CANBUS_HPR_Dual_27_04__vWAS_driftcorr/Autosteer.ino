...
//Added code to try and auto trim steering zero
// --- Field-Hardened Auto-Trim for Orbitrol Hydrostatic Slip ---
...
const float MAX_STRAIGHT_YAW_X100 = 80.0f;  // Max yaw rate threshold to consider "straight" 80 = .8 degrees per second
const float MIN_SPEED_FOR_TRIM = 3.5f;     // Minimum speed (km/h) to execute trim adjustments
const int32_t ACCUMULATOR_THRESHOLD = 500;  // Loop count threshold required to validate straight pass 500 = 10 seconds - control loop running at 50 hz
const float TRIM_GAIN = 0.0005f;            // Micro-adjust gain (5 parts in 10,000 to average out weaving)
...
// Modified code 5_18_2026 trying to auto trim Keya steering zero
if (keyaDetected && !steerConfig.SingleInputWAS)
    {
        ...
        // ========================================================
        // FIELD-HARDENED DYNAMIC ENCODER AUTO-TRIM LOGIC
        // ========================================================
        
        // Is the tractor broadly maintaining a straight heading at minimum speed?
        if ((abs(bnoData.angVel) < MAX_STRAIGHT_YAW_X100) && (gpsSpeed > MIN_SPEED_FOR_TRIM)) 
        {
            // Build confidence that we are working a straight pass. 
            // High-frequency steering corrections won't reset this instantly.
        ...
        {
            // If actively turning at a headland, decay confidence rapidly,
            // but don't instantly wipe it out over a single rough field bump.
            straightLineAccumulator -= 5; 
            if (straightLineAccumulator < 0) straightLineAccumulator = 0;
        }

        // Only apply trim if we have established high confidence in our baseline straight pass
        if (straightLineAccumulator >= ACCUMULATOR_THRESHOLD) 
        {
            // Calculate how far the current wheel angle is from our expected zero point
            float encoderDriftError = motorDeg - dynamicEncoderTrim;
            
            // Apply a microscopic low-pass adjustment. During active weave oscillations,
            // the error switches signs (+ and -) and cancels out perfectly.
            // Slow, one-sided hydrostatic drift will smoothly shift the trim variable.
            dynamicEncoderTrim += (encoderDriftError * TRIM_GAIN);
        }

        // Apply the dynamic drift correction to the motor tier
        ...
        // Optional debugging outputs for testing in the serial monitor:
        // Serial.print("Trim: "); Serial.print(dynamicEncoderTrim);
        // Serial.print(" Accum: "); Serial.print(straightLineAccumulator);
        // Serial.print(" SteerAngle: "); Serial.println(steerAngleActual);
    }
// end modified code

//ORIGINAL CODE use Keya encoder as simulated ADS1115 WAS
    // if (keyaDetected && !steerConfig.SingleInputWAS)
    // {
