...
void KeyaBus_Receive()
{
    // --- ADD THIS CAN BUS RECOVERY LINE ---
    // If the CAN controller crashed due to the motor turning off, 
    // this wakes the hardware back up so it can hear the motor again.
    Keya_Bus.events(); 
    ...
    if (Keya_Bus.read(KeyaBusReceiveData))
    {
        ...
        if (KeyaBusReceiveData.id == 0x07000001)
        {
            ...
            // Your original code continues exactly as it was below...
//            keyaSteeringPosition = (int16_t)((int16_t)KeyaBusReceiveData.buf[0] << 8 | (int16_t)KeyaBusReceiveData.buf[1]) * -1; [cite: 17]
