/*
   ...
   5/18/2026 - Enhanced drift correction with improved robustness:
   - Added Keya disconnect detection and trim reset
   - Implemented adaptive trim gain based on confidence
   - Added bounds checking on dynamic trim
   - Improved decay rate during turns
   - Added hysteresis for straight-line detection
   - Distance-based minimum threshold for baseline
...
const float TRIM_GAIN_HIGH = 0.002f;          // Aggressive gain once confidence established (4x faster)
const float MAX_TRIM_LIMIT = 5.0f;            // Maximum motor angle trim (±5°)
const elapsedMillis TRIM_DIAGNOSTIC_INTERVAL = 1000; // Log interval (ms)
static elapsedMillis trimDiagnosticTimer = 0;

// Enable/Disable diagnostic logging (set to 1 to enable)
#define ENABLE_TRIM_LOGGING 0
...
// ========================================================
// ENHANCED DYNAMIC ENCODER AUTO-TRIM FOR KEYA (5/18/2026)
// ========================================================
if (keyaDetected && !steerConfig.SingleInputWAS)
{
    // KEYA DISCONNECT DETECTION AND TRIM RESET
    // If Keya just disconnected, reset trim to safe state to prevent errors on reconnect
    ...
    // Get current steering position and calculate continuous degree value
    int16_t keyaPos;
    noInterrupts();
    ...
    // ========================================================
    // STRAIGHT-LINE DETECTION WITH HYSTERESIS
    ...
    // ========================================================
    // DISTANCE TRACKING FOR BASELINE CONFIDENCE
    // Requires minimum distance at constant speed, not just time
    // ========================================================
    if (isStraightLine && gpsSpeed > MIN_SPEED_FOR_TRIM)
    {
        // Update distance: convert speed (km/h) to m/s, multiply by dt (25ms)
        float metersPerLoop = (gpsSpeed * 0.27778f) * 0.025f;  // 0.27778 = 1000/3600 (km/h to m/s)
        distanceTraveledStraight += metersPerLoop;
        
        // Build confidence accumulator only after minimum distance is traveled
    ...
    {
        // ADAPTIVE DECAY RATE: Faster decay with more aggressive turning
        // This prevents trim from being applied during sharp turns
        ...
        distanceTraveledStraight = 0.0f;  // Reset distance counter
    }

    // ========================================================
    // ADAPTIVE TRIM GAIN BASED ON CONFIDENCE LEVEL
    // Conservative during buildup, aggressive once high confidence achieved
    // ========================================================
    if (straightLineAccumulator >= ACCUMULATOR_THRESHOLD) 
    {
        // HIGH CONFIDENCE ACHIEVED: Use aggressive gain for faster correction
        float encoderDriftError = motorDeg - dynamicEncoderTrim;
        dynamicEncoderTrim += (encoderDriftError * TRIM_GAIN_HIGH);
        
        #if ENABLE_TRIM_LOGGING
        if (trimDiagnosticTimer > TRIM_DIAGNOSTIC_INTERVAL) 
        {
            trimDiagnosticTimer = 0;
            Serial.print("[HIGH_CONF] Trim:"); Serial.print(dynamicEncoderTrim, 4);
            Serial.print(" DriftErr:"); Serial.print(encoderDriftError, 4);
            Serial.print(" MotorDeg:"); Serial.println(motorDeg, 4);
        }
        #endif
    }
    else if (straightLineAccumulator > 0)
    {
        // MEDIUM CONFIDENCE: Use conservative gain during buildup
        float encoderDriftError = motorDeg - dynamicEncoderTrim;
        ...
        {
            trimDiagnosticTimer = 0;
            Serial.print("[MED_CONF] Trim:"); Serial.print(dynamicEncoderTrim, 4);
            Serial.print(" Accum:"); Serial.print(straightLineAccumulator);
            Serial.print(" Dist:"); Serial.print(distanceTraveledStraight, 1);
            Serial.print("m YawRate:"); Serial.println(absYawRate, 1);
        }
        #endif
    }

    // ========================================================
    // TRIM SATURATION: Prevent runaway calibration
    // Clamp to reasonable physical limits (±5° motor angle)
    // ========================================================
    if (dynamicEncoderTrim > MAX_TRIM_LIMIT) 
    {
        dynamicEncoderTrim = MAX_TRIM_LIMIT;
        Serial.println("WARNING: Trim saturated at +MAX");
    }
    if (dynamicEncoderTrim < -MAX_TRIM_LIMIT) 
    {
        dynamicEncoderTrim = -MAX_TRIM_LIMIT;
    ...
    // Apply the dynamic drift correction to the motor angle
    float correctedMotorDeg = motorDeg - dynamicEncoderTrim;

    // Convert corrected motor → wheel angle to account for steering ratio
    float TireDeg = correctedMotorDeg / (steerSettings.steerSensorCounts/10); 

    // Adjust Tiredeg to take into account zero offset value from AOG
    // SteeringAngleActual is tire angle in degrees
    ...
    helloSteerPosition = (int16_t)(steerAngleActual * 100.0f);

    // Optional detailed debugging output (uncomment for troubleshooting)
    // Serial.print("Raw: "); Serial.print(keyaPos);
    // Serial.print(" Motor: "); Serial.print(motorDeg, 2);
    // Serial.print(" Trim: "); Serial.print(dynamicEncoderTrim, 4);
    // Serial.print(" Corrected: "); Serial.print(correctedMotorDeg, 2);
    // Serial.print(" Tire: "); Serial.print(TireDeg, 2);
    // Serial.print(" Final: "); Serial.println(steerAngleActual, 2);
}
// ========================================================
// END ENHANCED DYNAMIC TRIM LOGIC
// ========================================================

// use ADS1115 (fallback when Keya not detected or disabled)
else
{
    // Track that Keya is now off for next cycle
    wasKeyaDetected = false;
